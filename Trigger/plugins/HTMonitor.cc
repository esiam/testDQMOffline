...
void HTMonitor::bookHistograms(DQMStore::IBooker& ibooker, edm::Run const& iRun, edm::EventSetup const& iSetup) {
  ...
  //====== changed enum and switch with if =======
  if(quantity_ == "HT") {
    ...
    //=====================
      ...
      bookME(ibooker,
         ...
         histname,
         histtitle,
         nJets_HT_binning.nbins,
         nJets_HT_binning.xmin,
         nJets_HT_binning.xmax);
...
void HTMonitor::analyze(edm::Event const& iEvent, edm::EventSetup const& iSetup) {
  ...
  //=========added ===========
  int nJets = jets.size();   //jetSelection: PT>0
  float Pt_J1 = 0.0;
  float Pt_J2 = 0.0;
  float Phi_J1 = 10.0;
  float Phi_J2 = 10.0;
